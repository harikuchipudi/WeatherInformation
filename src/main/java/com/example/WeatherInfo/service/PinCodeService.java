...
public class PinCodeService {
    ...
    public ArrayList<PinCode> getGeoCoordinates(Long pinCode) throws Exception{

        ArrayList<PinCode> pinCodes = new ArrayList<>();
        //check if the pincode's longitude and latitiude are already availabel in the database
        Optional<PinCode> pinCodeOptional = pinCodeRepository.findByPinCode(pinCode);

        if (pinCodeOptional.isPresent()) {
            PinCode pinCodeFromTable = pinCodeOptional.get();
            pinCodes.add(pinCodeFromTable);
            return pinCodes;
        }
